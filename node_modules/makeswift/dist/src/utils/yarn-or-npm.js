...
exports.yarnOrNpm = void 0;
const fs = __importStar(require("fs"));
const path_1 = __importDefault(require("path"));
function yarnOrNpm(projectDirectory) {
    let lockFileFound = false;
    let currentDirectory = projectDirectory;
    let layers = 0;
    while (!lockFileFound && layers < 7) {
        if (fs.existsSync(path_1.default.join(currentDirectory, 'package-lock.json'))) {
            return 'npm';
        }
        if (fs.existsSync(path_1.default.join(currentDirectory, 'yarn.lock'))) {
            return 'yarn';
        }
